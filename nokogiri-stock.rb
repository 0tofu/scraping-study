...
class CompanyInfo
  ...
  attr_reader :name, :tickerCode, :category, :unit, :recentHighPrice, :recentLowPrice, :highPrice, :lowPrice, :price

  private

  def get_company_info()
    url = "#{BASE_URL}/profile/?code=#{@tickerCode}"
  ...
  def get_stock_info()
    url = "#{BASE_URL}/detail/?code=#{@tickerCode}"
    doc = get_nokogiri_doc(url)
...
puts "年初来安値：#{company.recentHighPrice}"
